...
// create an interupt for updating the time every second

void interupt() {
     if (TMR0IF_bit) {
          getTime();
          TMR0IF_bit = 0; // Clear Timer0 interrupt flag
          TMR0 = 0x06; // Load Timer0 with 6
          count++;
          if (count == 100) {
               count = 0;
          }
...
void main() {
     INTCON.GIE = 1; // Allow all interrupts
     INTCON.T0IE = 1; // Enable Timer0 interrupt
     OPTION_REG = 0x07; // Set Timer0 prescaler to 1:256
     TMR0 = 0x06; // Load Timer0 with 6
